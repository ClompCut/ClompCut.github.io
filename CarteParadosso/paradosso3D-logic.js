  ...
  if (card.type === 'creature') {
    ...
    if (targetIndex !== null) {
      ...
      if (card._currentHP <= 0) {
        ...
        // Remove card from hand without summoning
        gameState.playerManaSpent += card.cost;
      ...
      // Summon to empty field - but only if there are NO enemy creatures
      // If there are enemy creatures, we must attack one of them
      if (gameState.enemyField.length > 0) {
        addLog('Ci sono creature nemiche in campo! Devi attaccarne una.', '');
        return;
      }
