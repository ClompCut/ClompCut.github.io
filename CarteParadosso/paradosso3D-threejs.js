...
function createCreatureMesh(card, isPlayer) {
  ...
  const playerCount = summonedMeshes.filter(m => m.userData.isPlayer === true).length;
  const enemyCount = summonedMeshes.filter(m => m.userData.isPlayer === false).length;
  const count = isPlayer ? playerCount : enemyCount;
  const fieldLength = (window.GameLogic && window.GameLogic.gameState) ? 
    window.GameLogic.gameState()[isPlayer ? 'playerField' : 'enemyField'].length : 
    (isPlayer ? playerCount : enemyCount);
  const offset = (count - (fieldLength - 1)) * 2.5 - 2.5;
...
function cleanupDeadCreatures() {
  const gs = (window.GameLogic && window.GameLogic.gameState) ? window.GameLogic.gameState() : null;
  if (!gs) return;
  
  // Remove dead player creatures
  gs.playerField.forEach(card => {
    if (card._currentHP <= 0) {
      removeCreatureMesh(card, true);
      card._markedForRemoval = true;
    }
  });
  gs.playerField = gs.playerField.filter(c => !c._markedForRemoval);

  // Remove dead enemy creatures
  gs.enemyField.forEach(card => {
    if (card._currentHP <= 0) {
      removeCreatureMesh(card, false);
      card._markedForRemoval = true;
    }
  });
  gs.enemyField = gs.enemyField.filter(c => !c._markedForRemoval);
}
